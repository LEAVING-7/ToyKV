...
/* crc32 hash */
inline constexpr auto crc32(std::byte const* s, std::size_t len, std::uint32_t init = 0xFFFFFFFF) -> std::uint32_t
{
  std::uint32_t crc32val = init;
  for (std::size_t i = 0; i < len; i++) {
    crc32val = crc32_tab[(crc32val ^ std::to_integer<std::uint32_t>(s[i])) & 0xFF] ^ ((crc32val >> 8) & 0x00FFFFFF);
  }
  return crc32val ^ 0xFFFFFFFF;
}

inline constexpr auto crc32(std::span<std::byte const> s, std::uint32_t init = 0xFFFFFFFF) -> std::uint32_t
{
  return crc32(s.data(), s.size(), init);
}