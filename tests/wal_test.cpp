...
auto destroyWAL(Wal& wal) { fs::remove_all(wal.option().dirPath); }
...
TEST(WAL, Write)
{
  ...
  auto walResult = Wal::create(ops);
  ASSERT_TRUE(walResult);
...
auto testWriteAndIterate(Wal* wal, std::int32_t size, std::int32_t n)
{
  ...
  for (;;) {
    ...
    if (!data && data.error() == WalErr::EndOfSegments) {
      break;
    }
...
TEST(WAL, WriteLarge)
{
  ...
  ASSERT_TRUE(walResult);
  auto wal = std::move(walResult).value();
  testWriteAndIterate(wal.get(), 100000, 512);
...
TEST(WAL, WriteLarge2)
{
  ...
  testWriteAndIterate(wal.get(), 2000, 32 * 1024 * 3 + 10);
...
TEST(WAL, UseNewActiveSegment)
{
  ...
  testWriteAndIterate(wal.get(), 2000, 512);
...
TEST(WAL, Reader)
{
  ...
  auto validate = [](Wal& wal, std::int32_t size) {
    ...
    for (;;) {
      ...
      auto data = reader.next(pos);
      if (!data) {
        if (data.error() == WalErr::EndOfSegments) {
          break;
        } else {
  ...
  walResult = Wal::create(ops);
  ASSERT_TRUE(walResult);
  auto wal2 = std::move(walResult).value();
  validate(*wal2, size);
